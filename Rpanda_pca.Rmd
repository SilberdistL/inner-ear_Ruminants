---
title: "RPANDA pca data"
#this code fits model of trait evolution for which evolutionary rates depends on an environmental function, or more generally a time varying function using RPANDA package (Clavel et.al 2017) and the function fit_t_env. Evolutionary rates were exported using the package RRphylo on PCA data. 
---
...
pdf('RPANDA_PCA_VS_temperature_Pecora-Tragulina.pdf')
...
resultRT<-fit_t_env(treRT,RT, env_data=InfTemp, scale=TRUE)
    plot(resultRT, col="#313695", xlim=c(-45,0), ylim=c(0,0.0001), lwd=2,lty = 6)
...
write.csv(cbind(rt,pec), "AIC_results_PCA_Pecora_Tragulina.csv")
```
...
pdf('RPANDA_PCA_VS_temperature_Tragulina_Pecoran_families.pdf')
InfTemp[,1] <-InfTemp[,1]*(-1)
plot(InfTemp, xlim=c(-45,0), col="snow3", xlab="", ylab="", xaxt = "n", yaxt = "n",pch= "°" )
axis(side = 4)
mtext("Temperature", side=4)
...
write.csv(sig, "AIC_results_pca_evorates_Tragulina_Pecoran_families.csv")
```
